import { useState, useEffect, useRef } from 'react'
import {
  DICTIONARY_MAX_ALIASES,
  DICTIONARY_MAX_ENTRY_CHARS,
  DICTIONARY_MAX_ENTRY_WORDS
} from '../../../shared/constants'
import type { DictionaryEntry } from '../../../shared/types'

function parseAliases(input: string): string[] {
  return input
    .split(',')
    .map(alias => alias.trim())
    .filter(Boolean)
}

function IconClose() {
  return (
    <svg width="14" height="14" viewBox="0 0 20 20" fill="none">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M4.46967 4.46967C4.76256 4.17678 5.23744 4.17678 5.53033 4.46967L10 8.93934L14.4697 4.46967C14.7626 4.17678 15.2374 4.17678 15.5303 4.46967C15.8232 4.76256 15.8232 5.23744 15.5303 5.53033L11.0607 10L15.5303 14.4697C15.8232 14.7626 15.8232 15.2374 15.5303 15.5303C15.2374 15.8232 14.7626 15.8232 14.4697 15.5303L10 11.0607L5.53033 15.5303C5.23744 15.8232 4.76256 15.8232 4.46967 15.5303C4.17678 15.2374 4.17678 14.7626 4.46967 14.4697L8.93934 10L4.46967 5.53033C4.17678 5.23744 4.17678 4.76256 4.46967 4.46967Z"
        fill="currentColor"
      />
    </svg>
  )
}

export default function DictionaryPage() {
  const [entries, setEntries] = useState<DictionaryEntry[]>([])
  const [input, setInput] = useState('')
  const [aliasInput, setAliasInput] = useState('')
  const [feedback, setFeedback] = useState<string | null>(null)
  const inputRef = useRef<HTMLInputElement>(null)

  useEffect(() => {
    window.dictatelyAPI.getDictionary().then(setEntries)
  }, [])

  const handleAdd = async () => {
    const word = input.trim()
    if (!word) return

    const result = await window.dictatelyAPI.addWord({
      word,
      aliases: parseAliases(aliasInput)
    })
    if (result.status === 'added' && result.entry) {
      setEntries(prev => [result.entry, ...prev])
      setFeedback(null)
    } else {
      setFeedback(result.message ?? 'Unable to add that term.')
    }

    if (result.status === 'added') {
      setInput('')
      setAliasInput('')
    }

    inputRef.current?.focus()
  }

  const handleRemove = async (id: string) => {
    await window.dictatelyAPI.removeWord(id)
    setEntries(prev => prev.filter(e => e.id !== id))
  }

  const handleClear = async () => {
    await window.dictatelyAPI.clearDictionary()
    setEntries([])
    setFeedback(null)
  }

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') handleAdd()
  }

  return (
    <div className="dictionary-page">
      <div className="dictionary-header">
        <div>
          <h2>Personal Dictionary</h2>
          <p className="dictionary-subtitle">
            Add the exact term you want inserted, plus optional spoken aliases. Dictately uses them during transcription and in a correction pass after decoding.
          </p>
        </div>
        {entries.length > 0 && (
          <button className="btn-clear" onClick={handleClear}>Clear All</button>
        )}
      </div>

      <div className="dictionary-input-card">
        <div className="dictionary-input-group">
          <label className="dictionary-input-label">Term</label>
          <input
            ref={inputRef}
            type="text"
            className="dictionary-input"
            placeholder="e.g. PostgreSQL, Kubernetes, DALL-E"
            maxLength={DICTIONARY_MAX_ENTRY_CHARS}
            value={input}
            onChange={e => {
              setInput(e.target.value)
              if (feedback) setFeedback(null)
            }}
            onKeyDown={handleKeyDown}
          />
        </div>
        <div className="dictionary-input-group">
          <label className="dictionary-input-label">Aliases <span className="dictionary-input-optional">optional</span></label>
          <input
            type="text"
            className="dictionary-input"
            placeholder="Comma-separated spoken variants"
            value={aliasInput}
            onChange={e => {
              setAliasInput(e.target.value)
              if (feedback) setFeedback(null)
            }}
            onKeyDown={handleKeyDown}
          />
        </div>
        <div className="dictionary-input-footer">
          <span className="dictionary-limit">
            {DICTIONARY_MAX_ENTRY_WORDS} words max &middot; {DICTIONARY_MAX_ALIASES} aliases max
          </span>
          <button className="btn-add" onClick={handleAdd} disabled={!input.trim()}>
            Add Term
          </button>
        </div>
      </div>

      {feedback && (
        <div className="dictionary-feedback" role="status">
          {feedback}
        </div>
      )}

      {entries.length === 0 ? (
        <div className="dictionary-empty">
          <div className="dictionary-empty-icon">
            <svg width="40" height="40" viewBox="0 0 20 20" fill="none">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M10.43 19.9999C11.0553 20.0005 11.5687 20.0011 12.0425 19.801C12.4079 19.6468 12.7072 19.3912 13.0276 19.0754C13.0709 19.0331 13.1142 18.9896 13.1572 18.946C13.2106 18.892 13.2649 18.8365 13.3205 18.7798L16.803 15.2303C16.8581 15.1743 16.9119 15.1195 16.9644 15.0656C17.0081 15.0208 17.0518 14.9756 17.0943 14.9306C17.4037 14.6043 17.6539 14.2997 17.805 13.9281C18.001 13.4457 18.0005 12.923 17.9999 12.2854L17.9999 8.09515C17.9999 6.40174 17.9999 5.05339 17.8605 3.99652C17.7166 2.90602 17.4121 2.01239 16.719 1.30596C16.0257 0.59936 15.1485 0.288791 14.0779 0.142102C13.0408 -2.57607e-05 11.7177 -1.4201e-05 10.0564 -7.04314e-07L9.94344 -6.94438e-07C8.28219 -1.39006e-05 6.95908 -2.52291e-05 5.92191 0.142103C4.85139 0.288791 3.97413 0.599361 3.28085 1.30596C2.58774 2.01239 2.28324 2.90602 2.13939 3.99652C1.99997 5.05342 1.99999 6.40178 2 8.09523L2 11.9046C1.99999 13.5981 1.99998 14.9464 2.13939 16.0033C2.28324 17.0938 2.58774 17.9875 3.28085 18.6939C3.97413 19.4005 4.8514 19.7111 5.92191 19.8578C6.95912 19.9999 8.28226 19.9999 9.94358 19.9999L10.43 19.9999ZM11.0734 18.4633C11.0633 18.2993 11.0634 18.1178 11.0635 17.9433L11.0635 17.3374C11.0634 16.5111 11.0634 15.8153 11.1365 15.2613C11.214 14.6739 11.3857 14.1333 11.8134 13.6974C12.2412 13.2614 12.7721 13.0861 13.349 13.0071C13.8928 12.9326 14.5757 12.9326 15.3862 12.9326L15.9811 12.9326C16.1516 12.9325 16.3292 12.9325 16.4899 12.9427C16.4991 12.7707 16.5015 12.529 16.5015 12.1644L16.5015 8.15241C16.5015 6.389 16.5 5.14306 16.3755 4.19937C16.2539 3.27751 16.0275 2.75905 15.6587 2.38321C15.2901 2.00753 14.7819 1.77699 13.878 1.65313C12.9524 1.52629 11.7302 1.52466 9.99993 1.52467C8.26966 1.52467 7.04749 1.52629 6.12187 1.65313C5.21796 1.77699 4.70976 2.00753 4.34116 2.38321C3.9724 2.75905 3.74598 3.27751 3.62438 4.19937C3.49989 5.14306 3.49831 6.389 3.49831 8.15241L3.49831 11.8474C3.49831 13.6108 3.49989 14.8568 3.62438 15.8005C3.74598 16.7223 3.9724 17.2408 4.34116 17.6166C4.70976 17.9923 5.21796 18.2229 6.12187 18.3467C7.04749 18.4736 8.26966 18.4752 9.99993 18.4752L10.3109 18.4752C10.668 18.4752 10.9049 18.4727 11.0734 18.4633ZM12.5618 17.395L12.5618 17.39C12.5618 16.4975 12.5634 15.9048 12.6215 15.4642C12.6767 15.0455 12.7704 14.88 12.8737 14.7747C12.9769 14.6695 13.1387 14.5743 13.549 14.5181C13.981 14.4589 14.5622 14.4573 15.438 14.4573L15.4441 14.4573L12.5618 17.395Z"
                fill="currentColor"
              />
            </svg>
          </div>
          <p className="dictionary-empty-title">No terms yet</p>
          <p className="dictionary-empty-desc">
            Add names, acronyms, or technical terms that Whisper might miss.
          </p>
          <div className="dictionary-examples">
            <span className="dictionary-example" onClick={() => setInput('Dictately')}>Dictately</span>
            <span className="dictionary-example" onClick={() => setInput('Kubernetes')}>Kubernetes</span>
            <span className="dictionary-example" onClick={() => setInput('PostgreSQL')}>PostgreSQL</span>
          </div>
        </div>
      ) : (
        <div className="dictionary-list">
          <div className="dictionary-list-header">
            <span>{entries.length} {entries.length === 1 ? 'term' : 'terms'}</span>
          </div>
          {entries.map(entry => (
            <div key={entry.id} className="dictionary-entry">
              <div className="dictionary-entry-content">
                <span className="dictionary-word">{entry.word}</span>
                {entry.aliases.length > 0 && (
                  <span className="dictionary-aliases">
                    {entry.aliases.join(', ')}
                  </span>
                )}
              </div>
              <button
                className="dictionary-remove"
                onClick={() => handleRemove(entry.id)}
                title="Remove"
              >
                <IconClose />
              </button>
            </div>
          ))}
        </div>
      )}
    </div>
  )
}
